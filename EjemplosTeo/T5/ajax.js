...
function detectores() { //Envia con AJAX todos los enlaces del nav y la respuesta la pone en la etiqueta main
	enlaces = document.querySelectorAll("nav a");
	for (let enlace of enlaces) {
		enlace.addEventListener("click", function (event) {
			event.preventDefault();
			cargaAjax(event.target.getAttribute("href"), document.querySelector('main'));
		});
	}
	enviaForm();
}
